...
export type BoundingBox = {
  /** starts from 0 */
  class_idx: number; //class or category (multi label)
  /** normalized to [0,1] */
  x: number;
  y: number;
  width: number;
...
export type MultiLabelBoundingBox = BoundingBox & {
  image_id: number;
};

export type MultiLabelBoundingBoxWithKeypoints = BoundingBox & {
  keypoints: MultiLabelKeypoint[];
};
...
export type MultiLabelKeypoint =
  | Keypoint
  | {
      visibility: 0 | 1 | 2;
    };
...
export function parseMultiLabelString(
  task: "detect",
  annotation: {
    id: number;
    image_id: number;
...
export function parseMultiLabelString(
...
): BoundingBox | MultiLabelBoundingBoxWithKeypoints {
  if (task === "pose" && !annotation.keypoints) {
    throw new Error("Invalid input: task pose must include keypoints");
  }
  // parseBaseMultiLabelString()
  const { category_id: class_idx, bbox } = annotation;
  const x = +bbox[0];
  const y = +bbox[1];
  const width = +bbox[2];
  const height = +bbox[3];
  const multi_label_bounding_box: BoundingBox = {
    class_idx,
    x,
    y,
    width,
  ...
  const keypoints: MultiLabelKeypoint[] = [];
  if (task === "pose" && annotation.keypoints) {
    const step = 3;
    if (annotation.keypoints.length % 3 !== 0) {
      throw new Error(
        `Invalid pose label line: expect number of keypoints divisible by 3 but got ${annotation.keypoints.length} parts`
      );
    }

    for (let i = 0; i < annotation.keypoints.length; i += step) {
      const x = +annotation.keypoints[i];
      const y = +annotation.keypoints[i + 1];
      const visibility = annotation.keypoints[i + 2] as 0 | 1 | 2;
      const keypoint: MultiLabelKeypoint = { x, y, visibility };

      if (!isMultiLabelKeypoint(keypoint)) {
        throw new Error(`Invalid keypoint at position ${i}`);
      }
      keypoints.push(keypoint);
    }
    return {
      ...multi_label_bounding_box,
      keypoints
    }
  }
  return multi_label_bounding_box
}
